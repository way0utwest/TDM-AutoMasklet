...
$fullRestoreCreateScript = "$PSScriptRoot/postgresql/helper_scripts/CreatePGNorthwindFullRestore.sql"
$subsetCreateScript = "$PSScriptRoot/postgresql/helper_scripts/CreateNorthwindSubset.sql"
...
$subsetterOptionsFile = "$PSScriptRoot\postgresql\helper_scripts\rgsubset-options-northwind.json"

$winAuth = $true
$sourceConnectionString = "Server=$dbInstance;Port=$dbPort;Database=$sourceDb;Uid=$dbUser;Pwd=$dbPassword"
$targetConnectionString = "Server=$dbInstance;Port=$dbPort;Database=$targetDb;Uid=$dbUser;Pwd=$dbPassword"


Write-Output "Configuration:"
Write-Output "- dbinstance:             $dbinstance"
...
if ($backupPath) {
    ...
    $dbCreateSuccessful = Restore-StagingDatabasesFromBackup -WinAuth:$winAuth -dbinstance:$dbinstance -sourceDb:$sourceDb -targetDb:$targetDb -sourceBackupPath:$backupPath -SqlCredential:$SqlCredential
    ...
    $dbCreateSuccessful = New-SampleDatabases -WinAuth:$winAuth -dbinstance:$dbinstance -sourceDb:$sourceDb -targetDb:$targetDb -fullRestoreCreateScript:$fullRestoreCreateScript -subsetCreateScript:$subsetCreateScript -SqlCredential:$SqlCredential
    if ($dbCreateSuccessful){
        Write-Output "    Source and target databases created successfully."
    }
    else {
...
Write-Output "There should now be two databases on the $dbinstance server: $sourceDb and $targetDb"
